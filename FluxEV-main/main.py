...
def main_yahoo(args, data_dir):
    ...
    for _path in file_list:
        ...
        timestamp_test = timestamp[train_len:]  # 保存测试集的时间戳
        ...
        # 保存预测结果，包含时间戳
        all_predictions[file_name] = {
            'timestamp': timestamp_test,
            'true_labels': label_test,
            'predicted_labels': ret_test
        }
    ...
    # 保存所有预测结果到CSV文件
    for file_name, pred_dict in all_predictions.items():
        results_df = pd.DataFrame({
            'timestamp': pred_dict['timestamp'],
            'true_label': pred_dict['true_labels'],
            'predicted_label': pred_dict['predicted_labels']
        })
        results_df.to_csv(f'predictions_{file_name}.csv', index=False)
